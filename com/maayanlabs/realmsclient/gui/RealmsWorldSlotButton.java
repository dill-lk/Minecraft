...
import com.maayanlabs.realmsclient.util.RealmsTextureManager;
...
extends Button {
    ...
    public RealmsWorldSlotButton(int x, int y, int width, int height, int slotIndex, RealmsServer serverData, Button.OnPress onPress) {
        ...
        this.slotNameWidget = new StringWidget(Component.literal(this.state.slotName), Mayaan.getInstance().font);
    }
    ...
    public void renderContents(GuiGraphics graphics, int mouseX, int mouseY, float a) {
        ...
        if (this.state.activeSlot) {
            graphics.drawCenteredString(Mayaan.getInstance().font, RealmsMainScreen.getVersionComponent(this.state.slotVersion, this.state.compatibility.isCompatible()), x + this.width / 2, y + this.height + 2, -1);
        }
    }
