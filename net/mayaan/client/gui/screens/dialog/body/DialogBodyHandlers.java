...
import net.mayaan.client.gui.screens.dialog.body.DialogBodyHandler;
    ...
    implements DialogBodyHandler<ItemBody> {
        ...
        public LayoutElement createControls(DialogScreen<?> parent, ItemBody item) {
            ...
            if (item.description().isPresent()) {
                ...
                ItemDisplayWidget itemWidget = new ItemDisplayWidget(Mayaan.getInstance(), 0, 0, item.width(), item.height(), CommonComponents.EMPTY, displayStack, item.showDecorations(), item.showTooltip());
            ...
            return new ItemDisplayWidget(Mayaan.getInstance(), 0, 0, item.width(), item.height(), displayStack.getHoverName(), displayStack, item.showDecorations(), item.showTooltip());
        }
    }
}
