...
extends SingleQuadParticle {
    ...
    public TerrainParticle(ClientLevel level, double x, double y, double z, double xa, double ya, double za, BlockState blockState, BlockPos pos) {
        super(level, x, y, z, xa, ya, za, Mayaan.getInstance().getModelManager().getBlockStateModelSet().getParticleMaterial(blockState).sprite());
        this.pos = pos;
        this.gravity = 1.0f;
        this.rCol = 0.6f;
        this.gCol = 0.6f;
        this.bCol = 0.6f;
        BlockTintSource tintSource = Mayaan.getInstance().getBlockColors().getTintSource(blockState, 0);
        if (tintSource != null) {
