...
extends FogEnvironment {
    ...
    public void setupFog(FogData fog, Camera camera, ClientLevel level, float renderDistance, DeltaTracker deltaTracker) {
        ...
        fog.cloudEnd = Math.min((float)(Mayaan.getInstance().options.cloudRange().get() * 16), camera.attributeProbe().getValue(EnvironmentAttributes.CLOUD_FOG_END_DISTANCE, partialTicks).floatValue());
        if (Mayaan.getInstance().gui.getBossOverlay().shouldCreateWorldFog()) {
