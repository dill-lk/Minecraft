...
import net.mayaan.server.gui.PlayerListComponent;
import net.mayaan.server.gui.StatsComponent;
import org.slf4j.Logger;

public class MayaanServerGui
extends JComponent {
    private static final Font MONOSPACED = new Font("Monospaced", 0, 12);
    private static final Logger LOGGER = LogUtils.getLogger();
    private static final String TITLE = "Mayaan server";
    private static final String SHUTDOWN_TITLE = "Mayaan server - shutting down!";
    private final DedicatedServer server;
    private Thread logAppenderThread;
    private final Collection<Runnable> finalizers = Lists.newArrayList();
    private final AtomicBoolean isClosing = new AtomicBoolean();

    public static MayaanServerGui showFrameFor(final DedicatedServer server) {
        try {
        ...
        final MayaanServerGui gui = new MayaanServerGui(server);
        ...
        frame.addWindowListener(new WindowAdapter(){
            ...
            public void windowClosing(WindowEvent event) {
                if (!gui.isClosing.getAndSet(true)) {
                    frame.setTitle(MayaanServerGui.SHUTDOWN_TITLE);
                    server.halt(true);
                    gui.runFinalizers();
                }
            }
    ...
    private MayaanServerGui(DedicatedServer server) {
        this.server = server;
