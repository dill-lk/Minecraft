...
extends DataFix {
    private static final Map<String, Integer> MAP = (Map)DataFixUtils.make((Object)Maps.newHashMap(), map -> {
    ...
    public TypeRewriteRule makeRule() {
        ...
        Function<Typed, Typed> minecartUpdater = input -> this.updateBlockToBlockState((Typed<?>)input, "DisplayTile", "DisplayData", "DisplayState");
        Function<Typed, Typed> arrowUpdater = input -> this.updateBlockToBlockState((Typed<?>)input, "inTile", "inData", "inBlockState");
        Type oldProjectileType = DSL.and((Type)DSL.optional((Type)DSL.field((String)"inTile", (Type)DSL.named((String)References.BLOCK_NAME.typeName(), (Type)DSL.or((Type)DSL.intType(), NamespacedSchema.namespacedString())))), (Type)DSL.remainderType());
        ...
        return this.fixTypeEverywhereTyped("EntityBlockStateFix", inputSchema.getType(References.ENTITY), outputSchema.getType(References.ENTITY), input -> {
            input = this.updateEntity((Typed<?>)input, "minecraft:falling_block", this::updateFallingBlock);
            input = this.updateEntity((Typed<?>)input, "minecraft:enderman", input2 -> this.updateBlockToBlockState((Typed<?>)input2, "carried", "carriedData", "carriedBlockState"));
            input = this.updateEntity((Typed<?>)input, "minecraft:arrow", (Function<Typed<?>, Typed<?>>)arrowUpdater);
            input = this.updateEntity((Typed<?>)input, "minecraft:spectral_arrow", (Function<Typed<?>, Typed<?>>)arrowUpdater);
            input = this.updateEntity((Typed<?>)input, "minecraft:egg", (Function<Typed<?>, Typed<?>>)removeInTile);
            input = this.updateEntity((Typed<?>)input, "minecraft:ender_pearl", (Function<Typed<?>, Typed<?>>)removeInTile);
            input = this.updateEntity((Typed<?>)input, "minecraft:fireball", (Function<Typed<?>, Typed<?>>)removeInTile);
            input = this.updateEntity((Typed<?>)input, "minecraft:potion", (Function<Typed<?>, Typed<?>>)removeInTile);
            input = this.updateEntity((Typed<?>)input, "minecraft:small_fireball", (Function<Typed<?>, Typed<?>>)removeInTile);
            input = this.updateEntity((Typed<?>)input, "minecraft:snowball", (Function<Typed<?>, Typed<?>>)removeInTile);
            input = this.updateEntity((Typed<?>)input, "minecraft:wither_skull", (Function<Typed<?>, Typed<?>>)removeInTile);
            input = this.updateEntity((Typed<?>)input, "minecraft:xp_bottle", (Function<Typed<?>, Typed<?>>)removeInTile);
            input = this.updateEntity((Typed<?>)input, "minecraft:commandblock_minecart", (Function<Typed<?>, Typed<?>>)minecartUpdater);
            input = this.updateEntity((Typed<?>)input, "minecraft:minecart", (Function<Typed<?>, Typed<?>>)minecartUpdater);
            input = this.updateEntity((Typed<?>)input, "minecraft:chest_minecart", (Function<Typed<?>, Typed<?>>)minecartUpdater);
            input = this.updateEntity((Typed<?>)input, "minecraft:furnace_minecart", (Function<Typed<?>, Typed<?>>)minecartUpdater);
            input = this.updateEntity((Typed<?>)input, "minecraft:tnt_minecart", (Function<Typed<?>, Typed<?>>)minecartUpdater);
            input = this.updateEntity((Typed<?>)input, "minecraft:hopper_minecart", (Function<Typed<?>, Typed<?>>)minecartUpdater);
            input = this.updateEntity((Typed<?>)input, "minecraft:spawner_minecart", (Function<Typed<?>, Typed<?>>)minecartUpdater);
            return input;
        });
    }

    private Typed<?> updateFallingBlock(Typed<?> input) {
        Type oldType = DSL.optional((Type)DSL.field((String)"Block", (Type)DSL.named((String)References.BLOCK_NAME.typeName(), (Type)DSL.or((Type)DSL.intType(), NamespacedSchema.namespacedString()))));
        Type newType = DSL.optional((Type)DSL.field((String)"BlockState", (Type)DSL.named((String)References.BLOCK_STATE.typeName(), (Type)DSL.remainderType())));
        ...
        return input.update(oldType.finder(), newType, tile -> {
            int block = (Integer)tile.map(l -> (Integer)((Either)l.getSecond()).map(l2 -> l2, EntityBlockStateFix::getBlockId), r -> {
                Optional tileID = tag.get("TileID").asNumber().result();
                return tileID.map(Number::intValue).orElseGet(() -> tag.get("Tile").asByte((byte)0) & 0xFF);
            });
            int data = tag.get("Data").asInt(0) & 0xF;
            return Either.left((Object)Pair.of((Object)References.BLOCK_STATE.typeName(), BlockStateData.getTag(block << 4 | data)));
        }).set(DSL.remainderFinder(), (Object)tag.remove("Data").remove("TileID").remove("Tile"));
    }

    private Typed<?> updateBlockToBlockState(Typed<?> input, String oldFieldName, String dataName, String newFieldName) {
        Tag.TagType oldType = DSL.field((String)oldFieldName, (Type)DSL.named((String)References.BLOCK_NAME.typeName(), (Type)DSL.or((Type)DSL.intType(), NamespacedSchema.namespacedString())));
        Tag.TagType newType = DSL.field((String)newFieldName, (Type)DSL.named((String)References.BLOCK_STATE.typeName(), (Type)DSL.remainderType()));
        Dynamic tag = (Dynamic)input.getOrCreate(DSL.remainderFinder());
        return input.update(oldType.finder(), (Type)newType, tile -> {
            int block = (Integer)((Either)tile.getSecond()).map(l -> l, EntityBlockStateFix::getBlockId);
            int data = tag.get(dataName).asInt(0) & 0xF;
            return Pair.of((Object)References.BLOCK_STATE.typeName(), BlockStateData.getTag(block << 4 | data));
        }).set(DSL.remainderFinder(), (Object)tag.remove(dataName));
    }
    ...
    private Typed<?> updateEntity(Typed<?> input, String name, Function<Typed<?>, Typed<?>> function) {
        ...
        return input.updateTyped(DSL.namedChoice((String)name, (Type)oldType), newType, function);
    }
}
