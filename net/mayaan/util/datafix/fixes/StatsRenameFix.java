...
extends DataFix {
    ...
    private TypeRewriteRule createCriteriaRule() {
        ...
        OpticFinder customTypeFinder = DSL.namedChoice((String)"minecraft:custom", (Type)customFieldType);
        OpticFinder idFinder = DSL.fieldFinder((String)"id", NamespacedSchema.namespacedString());
        return this.fixTypeEverywhereTyped(this.name, inputType, outputType, input -> input.updateTyped(criteriaTypeFinder, stats -> stats.updateTyped(customTypeFinder, custom -> custom.update(idFinder, value -> this.renames.getOrDefault(value, (String)value)))));
    }
    ...
    private TypeRewriteRule createStatRule() {
        ...
        OpticFinder nameFinder = NamespacedSchema.namespacedString().finder();
        return this.fixTypeEverywhereTyped(this.name, inputType, outputType, input -> input.updateTyped(statsFinder, stats -> stats.updateTyped(customFinder, custom -> custom.update(nameFinder, value -> this.renames.getOrDefault(value, (String)value)))));
    }
}
