...
import net.mayaan.util.datafix.fixes.BlockEntityIdFix;
...
extends Schema {
    ...
    protected static final Hook.HookFunction ADD_NAMES = new Hook.HookFunction(){
        ...
        public <T> T apply(DynamicOps<T> ops, T value) {
            return V99.addNames(new Dynamic(ops, value), ITEM_TO_BLOCKENTITY, V99.ITEM_TO_ENTITY);
        }
    };
    ...
    public Type<?> getChoiceType(DSL.TypeReference type, String choiceName) {
        if (Objects.equals(type.typeName(), References.BLOCK_ENTITY.typeName())) {
    ...
    public Map<String, Supplier<TypeTemplate>> registerBlockEntities(Schema schema) {
        ...
        BlockEntityIdFix.ID_MAP.forEach((oldId, newId) -> map.put(newId, Objects.requireNonNull((Supplier)map.remove(oldId), () -> "Didn't find " + oldId + " in schema")));
        return map;
    }

    public void registerTypes(Schema schema, Map<String, Supplier<TypeTemplate>> entityTypes, Map<String, Supplier<TypeTemplate>> blockEntityTypes) {
        super.registerTypes(schema, entityTypes, blockEntityTypes);
        schema.registerType(true, References.BLOCK_ENTITY, () -> DSL.optionalFields((String)"components", (TypeTemplate)References.DATA_COMPONENTS.in(schema), (TypeTemplate)DSL.taggedChoiceLazy((String)"id", NamespacedSchema.namespacedString(), (Map)blockEntityTypes)));
