...
public enum FrontAndTop implements StringRepresentable
{
    ...
    public static FrontAndTop fromFrontAndTop(Direction front, Direction top) {
        return BY_TOP_FRONT[FrontAndTop.lookupKey(front, top)];
    }
    ...
    static {
        ...
        BY_TOP_FRONT = Util.make(new FrontAndTop[NUM_DIRECTIONS * NUM_DIRECTIONS], map -> {
            FrontAndTop[] arr$ = FrontAndTop.values();
            int len$ = arr$.length;
            for (int i$ = 0; i$ < len$; ++i$) {
                FrontAndTop value;
                map[FrontAndTop.lookupKey((Direction)value.front, (Direction)value.top)] = value = arr$[i$];
            }
        });
    }
}
