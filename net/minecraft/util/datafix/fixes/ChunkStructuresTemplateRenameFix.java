...
extends DataFix {
    ...
    private Dynamic<?> fixTag(Dynamic<?> structure, Dynamic<?> child) {
        ...
        if (RENAMES.containsKey((Object)id) && ((String)(data = (Pair)RENAMES.get((Object)id)).getFirst()).equals(child.get("id").asString(""))) {
            String template = child.get("Template").asString("");
            child = child.set("Template", child.createString((String)((ImmutableMap)data.getSecond()).getOrDefault((Object)template, (Object)template)));
        }
        return child;
    }
}
