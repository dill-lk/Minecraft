...
public class PlayerDataStorage {
    ...
    public void save(Player player) {
        try (ProblemReporter.ScopedCollector reporter = new ProblemReporter.ScopedCollector(player.problemPath(), LOGGER);){
            ...
            Path tmpFile = Files.createTempFile(playerDirPath, player.getStringUUID() + "-", ".dat", new FileAttribute[0]);
    ...
    private void backup(NameAndId nameAndId, String suffix) {
        ...
        try {
            ...
            LOGGER.warn("Failed to copy the player.dat file for {}", (Object)nameAndId.name(), (Object)e);
        }
    }

    private Optional<CompoundTag> load(NameAndId nameAndId, String suffix) {
        File realFile = new File(this.playerDir, String.valueOf(nameAndId.id()) + suffix);
    ...
    public Optional<CompoundTag> load(NameAndId nameAndId) {
        ...
        return optTag.or(() -> this.load(nameAndId, ".dat_old")).map(tag -> {
            ...
            tag = DataFixTypes.PLAYER.updateToCurrentVersion(this.fixerUpper, (CompoundTag)tag, version);
            return tag;
        });
    }
}
