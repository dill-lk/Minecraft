...
import net.mayaan.client.color.block.BlockTintCache;
...
import net.mayaan.client.multiplayer.ClientExplosionTracker;
...
import net.mayaan.client.renderer.LevelEventHandler;
...
import net.mayaan.client.resources.sounds.DirectionalSoundInstance;
import net.mayaan.client.resources.sounds.EntityBoundSoundInstance;
...
import net.mayaan.world.level.entity.TransientEntitySectionManager;
...
import net.mayaan.world.ticks.BlackholeTickAccess;
...
CacheSlot.Cleaner<ClientLevel> {
    ...
    public int calculateBlockTint(BlockPos pos, ColorResolver colorResolver) {
        int dist = Mayaan.getInstance().options.biomeBlendRadius().get();
        if (dist == 0) {
    ...
    public int getClientLeafTintColor(BlockPos pos) {
        BlockState state = this.getBlockState(pos);
        BlockTintSource tintSource = Mayaan.getInstance().getBlockColors().getTintSource(state, 0);
