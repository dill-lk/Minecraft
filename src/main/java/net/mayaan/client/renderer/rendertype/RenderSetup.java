...
import net.mayaan.client.renderer.rendertype.LayeringTransform;
import net.mayaan.client.renderer.rendertype.OutputTarget;
import net.mayaan.client.renderer.rendertype.TextureTransform;
...
public final class RenderSetup {
    ...
    public Map<String, TextureAndSampler> getTextures() {
        ...
        if (this.useOverlay) {
            result.put("Sampler1", new TextureAndSampler(Mayaan.getInstance().gameRenderer.overlayTexture().getTextureView(), RenderSystem.getSamplerCache().getClampToEdge(FilterMode.LINEAR)));
        }
        if (this.useLightmap) {
            result.put("Sampler2", new TextureAndSampler(Mayaan.getInstance().gameRenderer.lightmap(), RenderSystem.getSamplerCache().getClampToEdge(FilterMode.LINEAR)));
        }
