 ...
 *  com.mojang.authlib.minecraft.MayaanProfileTexture
 *  com.mojang.authlib.minecraft.MayaanProfileTexture$Type
 *  com.mojang.authlib.minecraft.MayaanProfileTextures
...
import com.mojang.authlib.minecraft.MayaanProfileTexture;
import com.mojang.authlib.minecraft.MayaanProfileTextures;
...
public class SkinManager {
    ...
    public SkinManager(Path skinsDirectory, final Services services, SkinTextureDownloader skinTextureDownloader, final Executor mainThreadExecutor) {
        ...
        this.skinTextures = new TextureCache(this, skinsDirectory, MayaanProfileTexture.Type.SKIN);
        this.capeTextures = new TextureCache(this, skinsDirectory, MayaanProfileTexture.Type.CAPE);
        this.elytraTextures = new TextureCache(this, skinsDirectory, MayaanProfileTexture.Type.ELYTRA);
            ...
            public CompletableFuture<Optional<PlayerSkin>> load(CacheKey key) {
                return ((CompletableFuture)CompletableFuture.supplyAsync(() -> {
                    ...
                    if (packedTextures == null) {
                        return MayaanProfileTextures.EMPTY;
                    }
                    MayaanProfileTextures textures = services.sessionService().unpackTextures(packedTextures);
                    ...
                    return textures;
                }, Util.backgroundExecutor().forName("unpackSkinTextures")).thenComposeAsync(textures -> this.this$0.registerTextures(key.profileId(), (MayaanProfileTextures)textures), mainThreadExecutor)).handle((playerSkin, throwable) -> {
                    if (throwable != null) {
    ...
    private CompletableFuture<PlayerSkin> registerTextures(UUID profileId, MayaanProfileTextures textures) {
        PlayerModelType model;
        CompletableFuture<ClientAsset.Texture> skinTexture;
        MayaanProfileTexture skinInfo = textures.skin();
        if (skinInfo != null) {
        ...
        MayaanProfileTexture capeInfo = textures.cape();
        ...
        MayaanProfileTexture elytraInfo = textures.elytra();
    ...
    private class TextureCache {
        ...
        private final MayaanProfileTexture.Type type;
        private final Map<String, CompletableFuture<ClientAsset.Texture>> textures;
        final /* synthetic */ SkinManager this$0;

        private TextureCache(SkinManager skinManager, Path root, MayaanProfileTexture.Type type) {
        ...
        public CompletableFuture<ClientAsset.Texture> getOrLoad(MayaanProfileTexture texture) {
        ...
        private CompletableFuture<ClientAsset.Texture> registerTexture(MayaanProfileTexture textureInfo) {
            ...
            return this.this$0.skinTextureDownloader.downloadAndRegisterSkin(textureId, file, textureInfo.getUrl(), this.type == MayaanProfileTexture.Type.SKIN);
        }

        private Identifier getTextureLocation(String textureHash) {
            String root = switch (this.type) {
                default -> throw new MatchException(null, null);
                case MayaanProfileTexture.Type.SKIN -> "skins";
                case MayaanProfileTexture.Type.CAPE -> "capes";
                case MayaanProfileTexture.Type.ELYTRA -> "elytra";
            };
