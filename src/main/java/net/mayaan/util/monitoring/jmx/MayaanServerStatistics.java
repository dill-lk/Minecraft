 ...
 *  org.slf4j.Logger
 */
package net.mayaan.util.monitoring.jmx;
...
public final class MayaanServerStatistics
implements DynamicMBean {
    ...
    private MayaanServerStatistics(MayaanServer server) {
        this.server = server;
        ...
        this.mBeanInfo = new MBeanInfo(MayaanServerStatistics.class.getSimpleName(), "metrics for dedicated server", mBeanAttributeInfos, null, null, new MBeanNotificationInfo[0]);
    }

    public static void registerJmxMonitoring(MayaanServer server) {
        try {
            ManagementFactory.getPlatformMBeanServer().registerMBean(new MayaanServerStatistics(server), new ObjectName("net.mayaan.server:type=Server"));
        }
